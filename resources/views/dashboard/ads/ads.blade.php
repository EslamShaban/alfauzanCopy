                ...
                <div class = "col-xs-3">
                    <a href = "{{route('offer_ads')}}"
                       class = "btn bg-teal-400 btn-block btn-float btn-float-lg correspondent">
                        <i class = " icon-station"></i> <span> العروض</span>
                    </a>
                </div>
                <div class = "col-xs-3">
        ...
        <table class = "table table-hover display" id = "example" data-order = "[]">
            ...
            @foreach($ads as $ad)
                ...
                <tr>
                    <td>
                        @if($ad->image)
                            <img src = "{{asset('dashboard/uploads/adss/'.$ad->image[0]->image)}}"
                                 style = "width:40px;height: 40px" class = "img-circle" alt = "">
                    ...
                    <td>{{round( $ad -> cost + (float)( $ad -> cost * $ad -> tax / 100.00 ) + (float)( $ad -> cost * $ad -> vat / 100.00 ),
                                           2 )}}</td>
                    <td>{{rate($ad->id)}}</td>
                    <td>{{$ad->is_rent ? 'للايجار':'للبيع'}}</td>
                    <td>{{$ad->area}}</td>
                    <td>{{count($ad->view)}}</td>
                    ...
                    <td>{{$ad->project->name_ar}}</td>
                    <td>{{$ad->adsCategory->name_ar}}</td>
            ...
            <div class = "modal-dialog" role = "document">
                <div class = "modal-content">
                    ...
                    <div class = "modal-body">
                        <div class = "row">
                            <form action = "{{route('add_ad')}}" method = "POST" id = "myForm"
                                ...
                                <div class = "row">
                                    ...
                                    <div class = "col-sm-6">
                                        <select name = "project" class = "form-control project"
                                            ...
                                            <option selected hidden disabled>--إختر المخطط --</option>
                                            @foreach($projects as $project)
                                                <option
                                                    value = "{{$project->id}}">{{$project->name_ar}}</option>
                                            @endforeach
                                        </select>
                                        <select name = "ads_category" class = "form-control cats"
                                                style = "margin-bottom: 5px" required>
                                    ...
                                    <div class = "col-sm-6">
                                        <label class = "checkbox" style = "margin-bottom: 0">
                                            <input type = "checkbox" name = "no_tax" value = "1">
                                            <i class = "icon-checkbox"></i>

                                            <label style = "padding-right: 0"> الغاء ضريبه الايجار</label>
                                        </label>
                                    </div>
                ...
                <div class = "modal-content">
                    ...
                    <div class = "modal-body">
                        <form action = "{{route('update_ad')}}" method = "post"
                            ...
                            <div class = "row">
                                <div class = "col-sm-6">
                                    <input type = "number" min = "0" step = ".01" name = "edit_cost"
                                           class = "form-control"
                                           placeholder = "السعر " required>
                                ...
                                <div class = "col-sm-6">
                                    <select name = "edit_project" id = "edit_project"
                                            class = "project form-control"
                                            style = "margin-bottom: 5px" required>
                                        <option hidden disabled>--إختر المخطط --</option>
                                        @foreach($projects as $project)
                                            <option value = "{{$project->id}}">{{$project->name_ar}}</option>
                                        @endforeach
                                    </select>
                                    <select name = "edit_ads_category" id = "edit_category"
                            ...
                            <div class = "col-sm-6">
                                <label class = "checkbox" style = "margin-bottom: 0">
                                    <input type = "checkbox" name = "no_tax" id="edit_no_tax" value = "1">
                                    <i class = "icon-checkbox"></i>
                                    <label style = "padding-right: 0"> الغاء ضريبه الايجار</label>
                                </label>
                            ...
                            <div class = "col-sm-12">
                                <input type = "text" name = "edit_video" class = "form-control"
                                       placeholder = "رابط مختصر الفيديو">
                            </div>
                            <div class = "col-sm-12">
										<textarea
                                            style = "margin-bottom: 5px"
                ...
                $.ajax( {
                            ...
                            success: function ( data ) {
                                ...
                                if ( data.status == 1 ) {
                                    ...
                                    alert( 'لا يمكن تغيير حالة هذا الاعلان' );
...
<script type = "text/javascript">
    ...
    function initializeEdit( ad ) {
        ...
        $( '#edit_category option' ).each( function () {
            if ( $( this ).val() == ad.ads_category_id ) {
                $( this ).attr( 'selected', '' )
            }
        } );
...
console.log(ad.no_tax);
        //block input check
        if ( ad.no_tax == 1 )
            $( '#edit_no_tax' ).prop( "checked", true );
        else
            $( '#edit_no_tax' ).prop( "checked", false );
    ...
    {{----}}
    {{--$( "input[name='lat']" ).val( '24.7135517' )--}}
    {{--$( "input[name='lng']" ).val( '46.67529569999999' )--}}
    {{--$( "input[name='lat']" ).val( '24.7135517' )--}}
    {{--$( "input[name='lng']" ).val( '46.67529569999999' )--}}
    {{--}--}}
